...
object Decoder
   ...
   * @group Decoding
   */
  implicit final val decodeJsonNumber: Decoder[JsonNumber] = (c: HCursor) =>
    c.value match {
      case Json.JNumber(v) => Right(v)
      case _               => Left(DecodingFailure(WrongTypeExpectation("number", c.value), c.history))
    }
